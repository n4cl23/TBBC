...
test.describe('canon consolidation',()=>{
  test('publishes the official taxonomy and causal timeline',async({request})=>{
    ...
    expect(canon.version).toBe('1.1.0');
    expect(canon.entities.filter((entity:{kind:string})=>entity.kind==='realm')).toHaveLength(6);
    expect(canon.entities.filter((entity:{kind:string})=>entity.kind==='event').map((event:{slug:string})=>event.slug)).toContain('covenant-of-guardians');
  ...
  test('resolves legacy names to a canonical slug',async({request})=>{
    ...
    expect(response.ok()).toBeTruthy();
    await expect(response.json()).resolves.toMatchObject({slug:'kingdom-of-the-abyss'});
  });
});
