...
try{
  const migrationRow=(await client.query('SELECT finished_at,rolled_back_at FROM "_prisma_migrations" WHERE migration_name=$1',[migration])).rows[0];
  if(!migrationRow||migrationRow.finished_at||migrationRow.rolled_back_at)throw new Error('Failed migration record no longer matches the audited state.');
  ...
  for(const table of requiredTables)if(!tables.has(table))throw new Error(`Expected partial table is absent: ${table}`);
  ...
  for(const column of requiredColumns)if(!columns.has(column))throw new Error(`Expected partial column is absent: ${column}`);
