...
export async function generateMetadata(): Promise<Metadata> {
  const locale = await requestLocale(), ui = copy[locale], canonical = localizedPath(locale, 'atlas', 'creatures');
  return { title: ui.title, description: ui.text, alternates: { canonical, languages: localizedAlternates('atlas', 'creatures') }, openGraph: { title: ui.title, description: ui.text, images: ['/images/bestiary/beasts-hero.webp'] } };
}

export default async function Page() {
  const locale = await requestLocale(), ui = copy[locale], creatures = localizedCreatures(locale);
